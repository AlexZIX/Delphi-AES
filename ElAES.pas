...
const
  ...
  LastForwardTable: array [0..255] of UInt32 = (
...
begin
  ...
  ExpandedKey[2] := PUInt32(@Key[8])^;
  ExpandedKey[3] := PUInt32(@Key[12])^;
  ExpandedKey[4] := PUInt32(@Key[16])^;
  ExpandedKey[5] := PUInt32(@Key[20])^;
  I := 0; J := 1;
  repeat
...
begin
  ...
  ExpandedKey[4] := PUInt32(@Key[16])^;
  ExpandedKey[5] := PUInt32(@Key[20])^;
  ExpandedKey[6] := PUInt32(@Key[24])^;
  ExpandedKey[7] := PUInt32(@Key[28])^;
  I := 0; J := 1;
  repeat
...
begin
  // initializing
  T0[0] := PUInt32(@InBuf[0])^ xor Key[0];
  T0[1] := PUInt32(@InBuf[4])^ xor Key[1];
  T0[2] := PUInt32(@InBuf[8])^ xor Key[2];
  T0[3] := PUInt32(@InBuf[12])^ xor Key[3];
  ...
  // finalizing
  PUInt32(@OutBuf[0])^ := T0[0]; PUInt32(@OutBuf[4])^ := T0[1];
  PUInt32(@OutBuf[8])^ := T0[2]; PUInt32(@OutBuf[12])^ := T0[3];
end;
...
begin
  // initializing
  T0[0] := PUInt32(@InBuf[0])^ xor Key[40];
  T0[1] := PUInt32(@InBuf[4])^ xor Key[41];
  T0[2] := PUInt32(@InBuf[8])^ xor Key[42];
  T0[3] := PUInt32(@InBuf[12])^ xor Key[43];
...
begin
  // initializing
  T0[0] := PUInt32(@InBuf[0])^ xor Key[48];
  T0[1] := PUInt32(@InBuf[4])^ xor Key[49];
  T0[2] := PUInt32(@InBuf[8])^ xor Key[50];
  T0[3] := PUInt32(@InBuf[12])^ xor Key[51];
...
begin
  // initializing
  T0[0] := PUInt32(@InBuf[0])^ xor Key[56];
  T0[1] := PUInt32(@InBuf[4])^ xor Key[57];
  T0[2] := PUInt32(@InBuf[8])^ xor Key[58];
  T0[3] := PUInt32(@InBuf[12])^ xor Key[59];
...
begin
  ...
  begin
    ...
    if Done < SizeOf(TempIn) then
      raise EStreamError.Create(SReadError);
    PUInt32(@TempIn[0])^ := PUInt32(@TempIn[0])^ xor PUInt32(@Vector[0])^;
    PUInt32(@TempIn[4])^ := PUInt32(@TempIn[4])^ xor PUInt32(@Vector[4])^;
    PUInt32(@TempIn[8])^ := PUInt32(@TempIn[8])^ xor PUInt32(@Vector[8])^;
    PUInt32(@TempIn[12])^ := PUInt32(@TempIn[12])^ xor PUInt32(@Vector[12])^;
    EncryptAES(TempIn, ExpandedKey, TempOut);
    Done := Dest.Write(TempOut, SizeOf(TempOut));
    if Done < SizeOf(TempOut) then
      raise EStreamError.Create(SWriteError);
...
begin
  ...
  begin
    ...
    Vector2 := TempIn;
    DecryptAES(TempIn, ExpandedKey, TempOut);
    PUInt32(@TempOut[0])^ := PUInt32(@TempOut[0])^ xor PUInt32(@Vector1[0])^;
    PUInt32(@TempOut[4])^ := PUInt32(@TempOut[4])^ xor PUInt32(@Vector1[4])^;
    PUInt32(@TempOut[8])^ := PUInt32(@TempOut[8])^ xor PUInt32(@Vector1[8])^;
    PUInt32(@TempOut[12])^ := PUInt32(@TempOut[12])^ xor PUInt32(@Vector1[12])^;
    Done := Dest.Write(TempOut, SizeOf(TempOut));
    if Done < SizeOf(TempOut) then
